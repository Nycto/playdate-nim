...
proc sharedOptions() =
  ...
  flag(
    "--no-auto-config", help = "Disables editing of config.nims, pdxinfo and .gitignore"
  )
  flag("--nim", help = "Calls the nim compiler directly instead of using nimble")

template defineBuild(
    name: string, kind: BuildKind, action: untyped, helpMessage: string
) =
...
var p = newParser:
  ...
  defineBuild(
    "simulator", SimulatorBuild, simulatorBuild, "Execute a build for the simulator"
  )
  defineBuild(
    "simulate", SimulatorBuild, runSimulator, "Execute a build and run the simulator"
  )
  ...
  defineBuild(
    "bundle", SimulatorBuild, bundlePDX,
    "Runs the Playdate compiler (pdc) and creates a pdx.zip",
  )
