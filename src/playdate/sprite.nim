...
type
  ...
  LCDSpriteCollisionFilterProc* = proc(
    sprite: LCDSprite, other: LCDSprite
  ): SpriteCollisionResponseType {.closure, raises: [].}
  LCDSpriteDrawFunction* =
    proc(sprite: LCDSprite, bounds: PDRect, drawRect: PDRect) {.closure, raises: [].}
...
proc remove*(this: LCDSprite) =
  privateAccess(PlaydateSprite)
  let dataNode =
    cast[ptr DoublyLinkedNodeObj[LCDSprite]](playdate.sprite.getUserdata(this.resource))
  if dataNode == nil:
    return
...
proc removeSprites*(this: ptr PlaydateSprite, sprites: openarray[LCDSprite]) =
  ...
  for i, s in sprites:
    let dataNode =
      cast[ptr DoublyLinkedNodeObj[LCDSprite]](this.getUserdata(s.resource))
    if dataNode == nil:
      return
    if dataNode.prev != nil:
      spritesData.remove(dataNode.prev.next)
...
proc privateUpdateFunction(sprite: LCDSpritePtr) {.cdecl, exportc, raises: [].} =
  privateAccess(PlaydateSprite)
  let spriteRef =
    cast[ptr DoublyLinkedNodeObj[LCDSprite]](playdate.sprite.getUserdata(sprite)).value
  spriteRef.updateFunction(spriteRef)

proc setUpdateFunction*(this: LCDSprite, update: LCDSpriteUpdateFunction) =
  privateAccess(PlaydateSprite)
  this.updateFunction = update
  playdate.sprite.setUpdateFunction(
    this.resource, if update != nil: privateUpdateFunction else: nil
  )

# --- Draw function.
proc privateDrawFunction(
    sprite: LCDSpritePtr, bounds: PDRect, drawRect: PDRect
...
proc setDrawFunction*(this: LCDSprite, draw: LCDSpriteDrawFunction) =
  ...
  playdate.sprite.setDrawFunction(
    this.resource, if draw != nil: privateDrawFunction else: nil
  )
...
# --- Collisions function.
proc privateCollisionResponse(
    sprite: LCDSpritePtr, other: LCDSpritePtr
): SpriteCollisionResponseType {.cdecl, exportc, raises: [].} =
  privateAccess(PlaydateSprite)
  let spriteRef =
    cast[ptr DoublyLinkedNodeObj[LCDSprite]](playdate.sprite.getUserdata(sprite)).value
  let otherRef =
    cast[ptr DoublyLinkedNodeObj[LCDSprite]](playdate.sprite.getUserdata(other)).value
  return spriteRef.collisionFunction(spriteRef, otherRef)

proc setCollisionResponseFunction*(
    this: LCDSprite, filter: LCDSpriteCollisionFilterProc
) =
  privateAccess(PlaydateSprite)
  this.collisionFunction = filter
  playdate.sprite.setCollisionResponseFunction(
    this.resource, if filter != nil: privateCollisionResponse else: nil
  )

proc sprite*(this: SpriteCollisionInfo): LCDSprite =
  privateAccess(PlaydateSprite)
  privateAccess(SpriteCollisionInfoPtr)
  let dataNode = cast[ptr DoublyLinkedNodeObj[LCDSprite]](playdate.sprite.getUserdata(
    this.spritePtr
  ))
  if dataNode == nil:
    return nil
...
proc other*(this: SpriteCollisionInfo): LCDSprite =
  ...
  privateAccess(SpriteCollisionInfoPtr)
  let dataNode =
    cast[ptr DoublyLinkedNodeObj[LCDSprite]](playdate.sprite.getUserdata(this.otherPtr))
  if dataNode == nil:
    return nil
  return dataNode.value
...
proc checkCollisions*(
    this: LCDSprite, goalX: float32, goalY: float32
  ...
  let collisionPtr = playdate.sprite.checkCollisions(
    this.resource,
    goalX.cfloat,
    goalY.cfloat,
    addr(actualX),
    addr(actualY),
    addr(collisionsCount),
  )
  let cArray = SDKArray[SpriteCollisionInfo](
    len: collisionsCount,
    data: cast[ptr UncheckedArray[SpriteCollisionInfo]](collisionPtr),
  )
  return (actualX: actualX.float32, actualY: actualY.float32, collisions: cArray)

proc moveWithCollisions*(
    this: LCDSprite, goalX: float32, goalY: float32
): tuple[actualX: float32, actualY: float32, collisions: SDKArray[SpriteCollisionInfo]] =
  privateAccess(PlaydateSprite)
  privateAccess(SDKArray)
  var actualX, actualY: cfloat
  var collisionsCount: cint
  let collisionPtr = playdate.sprite.moveWithCollisions(
    this.resource,
    goalX.cfloat,
    goalY.cfloat,
    addr(actualX),
...
proc querySpritesAtPoint*(this: ptr PlaydateSprite, x, y: float32): seq[LCDSprite] =
  ...
  let cArray = SDKArray[LCDSpritePtr](
    len: length, data: cast[ptr UncheckedArray[LCDSpritePtr]](sprites)
  )
  result = newSeq[LCDSprite](length)
  var i = 0
  for spr in cArray:
    result[i] =
      cast[ptr DoublyLinkedNodeObj[LCDSprite]](playdate.sprite.getUserdata(spr)).value
    i *= 1

proc querySpritesInRect*(
    this: ptr PlaydateSprite, x, y, width, height: float32
): seq[LCDSprite] =
  privateAccess(PlaydateSprite)
  privateAccess(SDKArray)
  var length: cint
  let sprites = playdate.sprite.querySpritesInRect(
    x.cfloat, y.cfloat, width.cfloat, height.cfloat, addr(length)
  )
  let cArray = SDKArray[LCDSpritePtr](
    len: length, data: cast[ptr UncheckedArray[LCDSpritePtr]](sprites)
  )
...
proc querySpritesAlongLine*(
    this: ptr PlaydateSprite, x1, y1, x2, y2: float32
): seq[LCDSprite] =
  privateAccess(PlaydateSprite)
  privateAccess(SDKArray)
  var length: cint
  let sprites = playdate.sprite.querySpritesAlongLine(
    x1.cfloat, y1.cfloat, x2.cfloat, y2.cfloat, addr(length)
  )
  let cArray = SDKArray[LCDSpritePtr](
    len: length, data: cast[ptr UncheckedArray[LCDSpritePtr]](sprites)
  )
...
proc querySpriteInfoAlongLine*(
    this: ptr PlaydateSprite, x1, y1, x2, y2: float32
): SDKArray[SpriteQueryInfo] =
  privateAccess(PlaydateSprite)
  privateAccess(SDKArray)
  var length: cint
  let queriesPtr = playdate.sprite.querySpriteInfoAlongLine(
    x1.cfloat, y1.cfloat, x2.cfloat, y2.cfloat, addr(length)
  )
  return SDKArray[SpriteQueryInfo](
    len: length, data: cast[ptr UncheckedArray[SpriteQueryInfo]](queriesPtr)
  )
...
proc setStencilImage*(this: LCDSprite, stencil: LCDBitmap, tile: bool) =
  privateAccess(PlaydateSprite)
  privateAccess(LCDBitmap)
  playdate.sprite.setStencilImage(
    this.resource, if stencil != nil: stencil.resource else: nil, if tile: 1 else: 0
  )
