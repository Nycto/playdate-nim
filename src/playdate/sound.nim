...
proc newAudioSample*(
    this: ptr PlaydateSound, path: string
): AudioSample {.raises: [IOError].} =
  privateAccess(PlaydateSound)
  privateAccess(PlaydateSoundSample)
...
proc privateFilePlayerFinishCallback(
    soundSource: SoundSourcePtr, userdata: pointer
) {.cdecl, raises: [].} =
  let filePlayer = cast[FilePlayer](userdata)
  if filePlayer.finishCallback != nil:
    filePlayer.finishCallback(filePlayer)

proc privateFilePlayerFadeVolumeCallback(
    soundSource: SoundSourcePtr, userdata: pointer
) {.cdecl, raises: [].} =
  let filePlayer = cast[FilePlayer](userdata)
...
proc setFinishCallback*(this: FilePlayer, callback: PDFilePlayerCallbackFunction) =
  ...
  if callback == nil:
    ...
    playdate.sound.fileplayer.setFinishCallback(
      this.resource, privateFilePlayerFinishCallback, cast[pointer](this)
    )
...
proc fadeVolume*(
    ...
    left, right: float32,
    len: int32,
    callback: PDFilePlayerCallbackFunction,
) =
  privateAccess(PlaydateSound)
  privateAccess(PlaydateSoundFileplayer)
  this.fadeVolumeCallback = callback
  if callback == nil:
    playdate.sound.fileplayer.fadeVolume(
      this.resource, left.cfloat, right.cfloat, len.cint, nil, nil
    )
  else:
    playdate.sound.fileplayer.fadeVolume(
      this.resource,
      ...
      right.cfloat,
      len.cint,
      privateFilePlayerFadeVolumeCallback,
      cast[pointer](this),
    )
...
proc `sample=`*(this: SamplePlayer, sample: AudioSample) =
  ...
  playdate.sound.sampleplayer.setSample(
    this.resource, if sample != nil: sample.resource else: nil
  )
  this.sample = sample

proc newSamplePlayer*(
    this: ptr PlaydateSound, path: string
): SamplePlayer {.raises: [IOError].} =
  privateAccess(PlaydateSound)
  privateAccess(PlaydateSoundSampleplayer)
  result = SamplePlayer(resource: this.sampleplayer.newPlayer(), finishCallback: nil)
...
proc privateSamplePlayerFinishCallback(
    soundSource: SoundSourcePtr, userdata: pointer
) {.cdecl, raises: [].} =
...
proc setFinishCallback*(this: SamplePlayer, callback: PDSamplePlayerCallbackFunction) =
  ...
  if callback == nil:
    ...
    playdate.sound.sampleplayer.setFinishCallback(
      this.resource, privateSamplePlayerFinishCallback, cast[pointer](this)
    )
...
proc setHeadphoneChangedCallback*(
    this: ptr PlaydateSound, changed: proc(headphone: bool, microphone: bool)
) =
  privateAccess(PlaydateSound)
  if changed == nil:
...
proc getHeadphoneState*(
    this: ptr PlaydateSound
): tuple[headphone: bool, microphone: bool] =
  privateAccess(PlaydateSound)
...
proc setLoops*(this: SoundSequence, loopstart, loopend, loops: int32) =
  ...
  playdate.sound.sequence.setLoops(
    this.resource, loopstart.cint, loopend.cint, loops.cint
  )
...
proc setCurrentStep*(this: SoundSequence, step, timeOffset, playNotes: int32) =
  privateAccess(PlaydateSoundSequence)
  playdate.sound.sequence.setCurrentStep(
    this.resource, step.cint, timeOffset.cint, playNotes.cint
  )
