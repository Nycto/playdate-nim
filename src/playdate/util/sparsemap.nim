...
proc toUInt(idx: DenseIdx): auto {.inline.} =
  uint32(idx) - 2

proc toDenseIdx(num: uint32): auto {.inline.} =
  DenseIdx(num + 2)

proc `==`(a, b: DenseIdx): bool {.inline.} =
  uint32(a) == uint32(b)

proc `=copy`[N: static int, K, V](
  a: var StaticSparseMap[N, K, V], b: StaticSparseMap[N, K, V]
) {.error.}

proc size*[N: static int, K, V](m: var StaticSparseMap[N, K, V]): auto {.inline.} =
  m.size
...
template find[N, K, V](
    m: var StaticSparseMap[N, K, V], key: K, exec: untyped
): untyped =
