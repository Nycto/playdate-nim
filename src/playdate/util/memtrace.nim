...
proc print(alloc: Allocation, title: cstring, printMem: bool = false) =
  cfprintf(
    cstderr, "%s (resized: %c, original size: %i, fenced: %c)\n", title,
    alloc.resized.yesNo, alloc.originalSize, alloc.protected.yesNo,
  )
  cfprintf(
    cstderr,
...
proc ord(p: pointer): auto =
  cast[uint64](p)

proc `+`(a: pointer, b: Natural): pointer =
  cast[pointer](cast[uint64](a) + b.uint64)

proc `-`(a: pointer, b: Natural): pointer =
  cast[pointer](cast[uint64](a) - b.uint64)

proc input(p: pointer): pointer =
  p - BUFFER

proc output(p: pointer): pointer =
  p + BUFFER

proc realSize(size: Natural): auto =
  size + BUFFER * 2
...
proc check(trace: var MemTrace) =
  if trace.totalAllocs mod 100 == 0:
    cfprintf(
      cstderr, "Allocations count: %i (active: %i)\n", trace.totalAllocs,
      trace.allocs.size,
    )

  for (_, alloc) in trace.allocs:
    if not alloc.protected and not alloc.reported and
        isInvalid(alloc.realPointer, alloc.realSize):
      alloc.reported = true
      alloc.print("CORRUPT! ")
...
proc memRange(alloc: Allocation): Slice[uint64] =
  return
    cast[uint64](alloc.realPointer) ..
    (cast[uint64](alloc.realPointer) + alloc.realSize.uint64)
...
proc traceAlloc*(
    trace: var MemTrace, alloc: PDRealloc, size: Natural
): pointer {.inline.} =
  trace.totalAllocs += 1
  trace.check
...
proc traceRealloc*(
    trace: var MemTrace, alloc: PDRealloc, p: pointer, newSize: Natural
): pointer {.inline.} =
  record[5]()
  trace.check
