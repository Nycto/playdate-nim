import
  std/[os, parsecfg, streams, strutils, strformat, times, osproc, options], nimbledump

type PdxInfo* = object ## Details used to populate the pdxinfo file
...
proc isSome(value: string): bool =
  not value.isEmptyOrWhitespace
...
proc toPdxInfo*(
  ...
  let bundleIdPkg =
    dump.author.toLower().replace(" ", "").replace("-", "").replace("_", "")
