...
proc exec*(command: string, args: varargs[string]) =
  ...
  let process = startProcess(
    command = command, args = args, options = {poUsePath, poParentStreams, poEchoCmd}
  )
...
proc updateGitIgnore(conf: PlaydateConf) =
  ...
  var toAdd = toHashSet(
    [conf.pdxName, conf.pdxName & ".zip", "source/pdxinfo", "source/pdex.*", "*.dSYM"]
  )
  ...
  if fileExists("config.nims"):
    for line in lines("config.nims"):
      if configFile in line:
        echo "config.nims already references ",
          configFile, "; skipping build configuration"
        return
...
proc mv(source, target: string) =
  ...
  if not source.fileExists and not source.dirExists:
    raise BuildFail.newException(
      fmt"Expecting the file '{source}' to exist, but it doesn't"
    )
