...
when defined(device):
  ...
  switch(
    "passC", "-mthumb -mcpu=cortex-m7 -mfloat-abi=hard -mfpu=fpv5-sp-d16 -D__FPU_USED=1"
  )
  ...
  switch(
    "passC",
    "-Wno-unused-but-set-variable -Wno-unused-label -Wno-parentheses -Wno-discarded-qualifiers -Wno-array-bounds",
  )

  switch("passL", "-nostartfiles")
  switch(
    "passL", "-mthumb -mcpu=cortex-m7 -mfloat-abi=hard -mfpu=fpv5-sp-d16 -D__FPU_USED=1"
  )
...
if useHostOS or nimbleTesting:
  ...
  patchFile(
    "stdlib", "malloc", currentSourcePath().parentDir() /../ "bindings" / "malloc"
  )
