...
proc getButtonState*(
    this: ptr PlaydateSys
): tuple[current: PDButtons, pushed: PDButtons, released: PDButtons] =
  privateAccess(PlaydateSys)
  var current, pushed, released: uint32
  this.getButtonState(
    cast[ptr PDButton](addr(current)),
    cast[ptr PDButton](addr(pushed)),
    cast[ptr PDButton](addr(released)),
  )
  return (
    current: cast[PDButtons](current),
    pushed: cast[PDButtons](pushed),
    released: cast[PDButtons](released),
  )

proc getAccelerometer*(
    this: ptr PlaydateSys
): tuple[x: float32, y: float32, z: float32] =
  privateAccess(PlaydateSys)
  var x, y, z: cfloat
...
type PDMenuItemCheckmarkCallbackFunction* =
  proc(menuItem: PDMenuItemCheckmark) {.raises: [].}

type PDMenuItemOptionsCallbackFunction* =
  proc(menuItem: PDMenuItemOptions) {.raises: [].}
...
proc addMenuItem*(
    this: ptr PlaydateSys, title: string, callback: PDMenuItemButtonCallbackFunction
): PDMenuItemButton =
  privateAccess(PDMenuItem)
  privateAccess(PlaydateSys)
  var menuItem = PDMenuItemButton(active: true)
  menuItem.callback = callback
  menuItem.resource = this.addMenuItem(
    title.cstring, privateMenuItemButtonCallback, cast[pointer](menuItem)
  )
  menuItems.add(menuItem)
  return menuItem

proc addCheckmarkMenuItem*(
    this: ptr PlaydateSys,
    title: string,
    checked: bool,
    callback: PDMenuItemCheckmarkCallbackFunction,
): PDMenuItemCheckmark =
  privateAccess(PDMenuItem)
  privateAccess(PlaydateSys)
  var menuItem = PDMenuItemCheckmark(active: true)
  menuItem.callback = callback
  menuItem.resource = this.addCheckmarkMenuItem(
    title.cstring,
    if checked: 1 else: 0,
    privateMenuItemCheckmarkCallback,
    cast[pointer](menuItem),
  )
  menuItems.add(menuItem)
  return menuItem

proc addOptionsMenuItem*(
    this: ptr PlaydateSys,
    title: string,
    options: seq[string],
    callback: PDMenuItemOptionsCallbackFunction,
): PDMenuItemOptions =
  privateAccess(PDMenuItem)
  ...
  menuItem.callback = callback
  let cOptions = options.map(
    proc(x: string): cstring =
      return x.cstring
  )
  menuItem.resource = this.addOptionsMenuItem(
    title.cstring,
    cast[ConstCharPtr](unsafeAddr(cOptions[0])),
    cOptions.len.cint,
    privateMenuItemOptionsCallback,
    cast[pointer](menuItem),
  )
  menuItems.add(menuItem)
  return menuItem
...
proc setMenuImage*(this: ptr PlaydateSys, image: LCDBitmap, xOffset: int32 = 0) =
  privateAccess(PlaydateSys)
  this.setMenuImage(image.resource, xOffset.cint)
...
proc privateSerialMessageCallback(msg: ConstChar) {.cdecl, raises: [].} =
  serialMsgCallback($msg)
