...
proc newManagedBitmap(point: LCDBitmapPtr): auto =
...
proc getContext*(this: LCDVideoPlayer): LCDBitmap =
    ...
    if this.context.isNil or this.context.resource != bitmapPtr:
        this.context = newUnmanagedBitmap(bitmapPtr)
    return this.context
...
proc newBitmap*(this: ptr PlaydateGraphics, width: int, height: int, color: LCDColor): LCDBitmap =
    privateAccess(PlaydateGraphics)
    return newManagedBitmap(this.newBitmap(width.cint, height.cint, color.convert))

proc newBitmap*(this: ptr PlaydateGraphics, path: string): LCDBitmap {.raises: [IOError]} =
    privateAccess(PlaydateGraphics)
    var err: ConstChar = nil
    let bitmap = newManagedBitmap(this.loadBitmap(path, addr(err)))
...
proc copy*(this: LCDBitmap): LCDBitmap =
    privateAccess(PlaydateGraphics)
    return newManagedBitmap(playdate.graphics.copyBitmap(this.resource))
...
proc rotated*(this: LCDBitmap, rotation: float32, xScale: float32, yScale: float32):
    ...
    let bitmap = newManagedBitmap(playdate.graphics.rotatedBitmap(this.resource, rotation.cfloat, xScale.cfloat, yScale.cfloat,
        addr(allocatedSize)))
...
proc getBitmap*(this: LCDBitmapTable, index: int): LCDBitmap =
    ...
    if resource != nil:
        return newUnmanagedBitmap(resource)
...
proc getDebugBitmap*(this: ptr PlaydateGraphics): LCDBitmap =
    privateAccess(PlaydateGraphics)
    return newManagedBitmap(this.getDebugBitmap()) # Who should manage this memory? Not clear. Auto-managed.

proc copyFrameBufferBitmap*(this: ptr PlaydateGraphics): LCDBitmap =
    privateAccess(PlaydateGraphics)
    return newManagedBitmap(this.copyFrameBufferBitmap())
...
proc getDisplayBufferBitmap*(this: ptr PlaydateGraphics): LCDBitmap =
    privateAccess(PlaydateGraphics)
    return newUnmanagedBitmap(this.getDisplayBufferBitmap())
...
proc getBitmapMask*(this: LCDBitmap): LCDBitmap =
    privateAccess(PlaydateGraphics)
    return newUnmanagedBitmap(playdate.graphics.getBitmapMask(this.resource)) # Who should manage this memory? Not clear. Not auto-managed right now.
