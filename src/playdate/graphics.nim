...
proc newVideoPlayer*(
    this: ptr PlaydateVideo, path: string
): LCDVideoPlayer {.raises: [IOError].} =
  privateAccess(PlaydateVideo)
...
proc setContext*(
    this: LCDVideoPlayer, context: LCDBitmap
) {.raises: [CatchableError].} =
  privateAccess(PlaydateVideo)
  if playdate.graphics.video.setContext(this.resource, context.resource) == 0:
...
proc getInfo*(
  ...
  width: int, height: int, frameRate: float32, frameCount: int, currentFrame: int
] =
  privateAccess(PlaydateVideo)
  var width, height, frameCount, currentFrame: cint
  var frameRate: cfloat
  playdate.graphics.video.getInfo(
    this.resource,
    addr(width),
    addr(height),
    addr(frameRate),
...
proc drawTiled*(
    this: LCDBitmap, x: int, y: int, width: int, height: int, flip: LCDBitmapFlip
) =
  privateAccess(PlaydateGraphics)
  playdate.graphics.tileBitmap(
    this.resource, x.cint, y.cint, width.cint, height.cint, flip
  )

proc drawScaled*(this: LCDBitmap, x: int, y: int, xScale: float32, yScale: float32) =
  privateAccess(PlaydateGraphics)
  playdate.graphics.drawScaledBitmap(
    this.resource, x.cint, y.cint, xScale.cfloat, yScale.cfloat
  )
...
proc drawText*(
  ...
  return playdate.graphics.drawText(
    text.cstring, len(text).csize_t, encoding, x.cint, y.cint
  ).int

proc newBitmap*(
    this: ptr PlaydateGraphics, width: int, height: int, color: LCDColor
): LCDBitmap =
  privateAccess(PlaydateGraphics)
  return bitmapRef(this.newBitmap(width.cint, height.cint, color.convert))

proc newBitmap*(
    this: ptr PlaydateGraphics, path: string
): LCDBitmap {.raises: [IOError].} =
  privateAccess(PlaydateGraphics)
  var err: ConstChar = nil
  let bitmap = bitmapRef(this.loadBitmap(path, addr(err)))
...
proc copy*(this: LCDBitmap): LCDBitmap =
  privateAccess(PlaydateGraphics)
  return bitmapRef(playdate.graphics.copyBitmap(this.resource))
...
proc index(x, y, rowbytes: int): int =
...
proc getSize*(this: LCDBitmap): tuple[width: int, height: int] =
  ...
  playdate.graphics.getBitmapData(
    this.resource, addr(width), addr(height), nil, nil, nil
  )
...
proc rotated*(
    this: LCDBitmap, rotation: float32, xScale: float32, yScale: float32
): tuple[bitmap: LCDBitmap, allocatedSize: int] =
  privateAccess(PlaydateGraphics)
  var allocatedSize: cint
  let bitmap = bitmapRef(
    playdate.graphics.rotatedBitmap(
      this.resource, rotation.cfloat, xScale.cfloat, yScale.cfloat, addr(allocatedSize)
    )
  )
  return (bitmap, allocatedSize.int)

proc rotated*(
    this: LCDBitmap, rotation: float32, scale: float32
...
proc newBitmapTable*(
    this: ptr PlaydateGraphics, count: int, width: int, height: int
): LCDBitmapTable =
  privateAccess(PlaydateGraphics)
  privateAccess(LCDBitmapTable)
  return
    LCDBitmapTable(resource: this.newBitmapTable(count.cint, width.cint, height.cint))

proc newBitmapTable*(
    this: ptr PlaydateGraphics, path: string
): LCDBitmapTable {.raises: [IOError].} =
  privateAccess(PlaydateGraphics)
  var err: ConstChar = nil
...
proc getBitmap*(this: LCDBitmapTable, index: int): LCDBitmap =
  ...
  if resource != nil:
    return bitmapPtr(resource)
  return default(LCDBitmap)
...
proc getGlyphKerning*(this: LCDFontGlyph, glyphCode: char, nextCode: char): int =
  privateAccess(PlaydateGraphics)
  privateAccess(LCDFontGlyph)
  return playdate.graphics.getGlyphKerning(
    this.resource, glyphCode.uint32, nextCode.uint32
  ).int
...
proc getTextWidth*(
  ...
  return playdate.graphics.getTextWidth(
    this.resource, text.cstring, len.csize_t, encoding, tracking.cint
  )
...
proc getFrame*(this: ptr PlaydateGraphics): DisplayFrame =
  privateAccess(PlaydateGraphics)
  return cast[DisplayFrame](this.getFrame())
    # Who should manage this memory? Not clear. Not auto-managed right now.

proc getDisplayFrame*(this: ptr PlaydateGraphics): DisplayFrame =
  privateAccess(PlaydateGraphics)
  return cast[DisplayFrame](this.getDisplayFrame())
    # Who should manage this memory? Not clear. Not auto-managed right now.

proc width*(frame: DisplayFrame): auto {.inline.} =
  ## Return the width of the display frame buffer.
  LCD_COLUMNS

proc height*(frame: DisplayFrame): auto {.inline.} =
...
proc copyFrameBufferBitmap*(this: ptr PlaydateGraphics): LCDBitmap =
  privateAccess(PlaydateGraphics)
  return bitmapRef(this.copyFrameBufferBitmap())

proc createPattern*(
    this: ptr PlaydateGraphics, bitmap: LCDBitmap, x: int, y: int
): LCDPattern =
  privateAccess(PlaydateGraphics)
...
proc fillPolygon*[Int32x2](
    this: ptr PlaydateGraphics,
    points: openArray[Int32x2],
    color: LCDColor,
    fillRule: LCDPolygonFillRule,
) =
  when sizeof(Int32x2) != sizeof(int32) * 2:
    {.error: "size of points is not sizeof(int32) * 2".}

  privateAccess(PlaydateGraphics)
  this.fillPolygon(
    points.len.cint, cast[ptr cint](unsafeAddr(points[0])), color.convert, fillRule
  )
...
proc getDisplayBufferBitmap*(this: ptr PlaydateGraphics): LCDBitmap =
  privateAccess(PlaydateGraphics)
  return bitmapPtr(this.getDisplayBufferBitmap())
...
proc drawRotated*(
  ...
  playdate.graphics.drawRotatedBitmap(
    this.resource, x.cint, y.cint, rotation.cfloat, centerX.cfloat, centerY.cfloat,
    xScale.cfloat, yScale.cfloat,
  )

proc width*(this: LCDBitmap): int =
  this.getSize.width

proc height*(this: LCDBitmap): int =
  this.getSize.height
...
proc getBitmapMask*(this: LCDBitmap): LCDBitmap =
  privateAccess(PlaydateGraphics)
  return bitmapRef(playdate.graphics.getBitmapMask(this.resource))
...
proc getTextSize*(
    this: LCDFont, text: string, lineHeightAdjustment: int = 0
...
proc drawTextAligned*(
  ...
  var drawX = x
  var drawY = y
  let lineHeight = font.getFontHeight().int + lineHeightAdjustment
    #+ this.getTextLeading() # missing function
...
proc layoutTextInRect(
    this: LCDFont,
    text: string,
    x, y, width, height: int,
    lineHeightAdjustment: int = 0,
    truncationString: string = "...",
    alignment: TextAlignment = kTextAlignmentLeft,
    draw: bool = true,
  ...
  block main:
    for line in lines:
      ...
      if currentLine.len > 0:
        currentLine.delete(0 ..< currentLine.len)
  ...
  return TextInRectResult(
    width: maxLineWidth, height: drawY - y + fontHeight, textWasTruncated: truncated
  )
...
proc drawTextInRect*(
  ...
  result = layoutTextInRect(
    this, text, x, y, width, height, lineHeightAdjustment, truncationString, alignment,
    true,
  )

  playdate.graphics.setFont(previousFont)

proc getTextSizeInRect*(
    this: LCDFont,
    text: string,
    x, y, width, height: int,
    lineHeightAdjustment: int = 0,
