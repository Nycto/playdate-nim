...
import
  graphics, system, file, sprite, display, sound, scoreboards, lua, json, utils, types,
  nineslice
export
  graphics, system, file, sprite, display, sound, scoreboards, lua, json, utils, types,
  nineslice
...
macro initSDK*() =
  return quote:
    ...
    proc eventHandler(
        playdateAPI: ptr PlaydateAPI, event: PDSystemEvent, arg: uint32
    ): cint {.cdecl, exportc.} =
...
when not defined(simulator) and defined(release):
  ...
  proc write(
      handle: cint, data: ptr cchar, size: cint
  ): cint {.cdecl, exportc: "_write".} =
    return -1
