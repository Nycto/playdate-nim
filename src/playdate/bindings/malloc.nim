...
when defined(memProfiler):
  ...
  template rawRealloc(p, size): untyped =
    pdrealloc(p, size)

  template rawDealloc(p) =
  ...
  template rawAlloc(size): untyped =
    traceAlloc(trace, pdrealloc, size)

  template rawRealloc(p, size): untyped =
    traceRealloc(trace, pdrealloc, p, size)

  template rawDealloc(p) =
    traceDealloc(trace, pdrealloc, p)
...
proc allocSharedImpl(size: Natural): pointer {.used.} =
  allocImpl(size)

proc allocShared0Impl(size: Natural): pointer {.used.} =
  alloc0Impl(size)

proc reallocSharedImpl(p: pointer, newSize: Natural): pointer {.used.} =
  reallocImpl(p, newSize)

proc reallocShared0Impl(p: pointer, oldsize, newSize: Natural): pointer {.used.} =
  realloc0Impl(p, oldSize, newSize)

proc deallocSharedImpl(p: pointer) {.used.} =
  deallocImpl(p)

proc getOccupiedMem(): int {.used.} =
  discard

proc getFreeMem(): int {.used.} =
  discard

proc getTotalMem(): int {.used.} =
  discard

proc deallocOsPages() {.used.} =
  discard
