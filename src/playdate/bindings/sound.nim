...
type SequenceFinishedCallback =
  proc(soundSource: SoundSequencePtr, userdata: pointer) {.cdecl.}

type PDSndCallbackProcRaw {.importc: "sndCallbackProc", header: "pd_api.h".} =
  proc(soundSource: SoundSourcePtr, userdata: pointer) {.cdecl.}

type PlaydateSoundFileplayer {.
  importc: "const struct playdate_sound_fileplayer", header: "pd_api.h", bycopy
.} = object
  newPlayer {.importc: "newPlayer".}: proc(): FilePlayerPtr {.cdecl, raises: [].}
  freePlayer {.importc: "freePlayer".}:
    proc(player: FilePlayerPtr) {.cdecl, raises: [].}
  loadIntoPlayer {.importc: "loadIntoPlayer".}:
    proc(player: FilePlayerPtr, path: cstring): cint {.cdecl, raises: [].}
  # setBufferLength* {.importc: "setBufferLength".}: proc (player: ptr FilePlayer;
  #     bufferLen: cfloat) {.cdecl.}
  play {.importc: "play".}:
    proc(player: FilePlayerPtr, repeat: cint): cint {.cdecl, raises: [].}
  isPlaying {.importc: "isPlaying".}:
    proc(player: FilePlayerPtr): cint {.cdecl, raises: [].}
  pause {.importc: "pause".}: proc(player: FilePlayerPtr) {.cdecl, raises: [].}
  stop {.importc: "stop".}: proc(player: FilePlayerPtr) {.cdecl, raises: [].}
  setVolume {.importc: "setVolume".}:
    proc(player: FilePlayerPtr, left: cfloat, right: cfloat) {.cdecl, raises: [].}
  getVolume {.importc: "getVolume".}: proc(
    player: FilePlayerPtr, left: ptr cfloat, right: ptr cfloat
  ) {.cdecl, raises: [].}
  getLength {.importc: "getLength".}:
    proc(player: FilePlayerPtr): cfloat {.cdecl, raises: [].}
  setOffset {.importc: "setOffset".}:
    proc(player: FilePlayerPtr, offset: cfloat) {.cdecl, raises: [].}
  ...
  setFinishCallback* {.importc: "setFinishCallback".}: proc(
    player: FilePlayerPtr, callback: PDSndCallbackProcRaw, userdata: pointer = nil
  ) {.cdecl, raises: [].}
  # setLoopCallback* {.importc: "setLoopCallback".}: proc (player: ptr FilePlayer;
  #     callback: SndCallbackProc) {.cdecl.}
  getOffset {.importc: "getOffset".}:
    proc(player: FilePlayerPtr): cfloat {.cdecl, raises: [].}
  ...
  fadeVolume* {.importc: "fadeVolume".}: proc(
    player: FilePlayerPtr,
    left: cfloat,
    right: cfloat,
    len: cint,
    finishCallback: PDSndCallbackProcRaw,
...
type PlaydateSoundSample {.
  importc: "const struct playdate_sound_sample", header: "pd_api.h"
.} = object
  newSampleBuffer {.importc: "newSampleBuffer".}:
    proc(byteCount: cint): AudioSamplePtr {.cdecl, raises: [].}
  loadIntoSample {.importc: "loadIntoSample".}:
    proc(sample: AudioSamplePtr, path: cstring): cint {.cdecl, raises: [].}
  ...
  freeSample {.importc: "freeSample".}:
    proc(sample: AudioSamplePtr) {.cdecl, raises: [].}
  getLength {.importc: "getLength".}:
    proc(sample: AudioSamplePtr): cfloat {.cdecl, raises: [].}

# type PlaydateSoundSample* = ptr PlaydateSoundSampleRaw

type PlaydateSoundSampleplayer {.
  importc: "const struct playdate_sound_sampleplayer", header: "pd_api.h"
.} = object
  newPlayer {.importc: "newPlayer".}: proc(): SamplePlayerPtr {.cdecl, raises: [].}
  freePlayer {.importc: "freePlayer".}:
    proc(player: SamplePlayerPtr) {.cdecl, raises: [].}
  setSample {.importc: "setSample".}:
    proc(player: SamplePlayerPtr, sample: AudioSamplePtr) {.cdecl, raises: [].}
  play {.importc: "play".}: proc(
    player: SamplePlayerPtr, repeat: cint, rate: cfloat
  ): cint {.cdecl, raises: [].}
  isPlaying {.importc: "isPlaying".}:
    proc(player: SamplePlayerPtr): cint {.cdecl, raises: [].}
  stop {.importc: "stop".}: proc(player: SamplePlayerPtr) {.cdecl, raises: [].}
  setVolume {.importc: "setVolume".}:
    proc(player: SamplePlayerPtr, left: cfloat, right: cfloat) {.cdecl, raises: [].}
  getVolume {.importc: "getVolume".}: proc(
    player: SamplePlayerPtr, left: ptr cfloat, right: ptr cfloat
  ) {.cdecl, raises: [].}
  getLength {.importc: "getLength".}:
    proc(player: SamplePlayerPtr): cfloat {.cdecl, raises: [].}
  setRate {.importc: "setRate".}:
    proc(player: SamplePlayerPtr, rate: cfloat) {.cdecl, raises: [].}
  getRate {.importc: "getRate".}:
    proc(player: SamplePlayerPtr): cfloat {.cdecl, raises: [].}
  setPlayRange* {.importc: "setPlayRange".}:
    proc(player: SamplePlayerPtr, start: cint, `end`: cint) {.cdecl, raises: [].}
  setFinishCallback* {.importc: "setFinishCallback".}: proc(
    player: SamplePlayerPtr, callback: PDSndCallbackProcRaw, userdata: pointer = nil
  ) {.cdecl, raises: [].}
  ...
  getOffset* {.importc: "getOffset".}:
    proc(player: SamplePlayerPtr): cfloat {.cdecl, raises: [].}
  setOffset {.importc: "setOffset".}:
    proc(player: SamplePlayerPtr, offset: cfloat) {.cdecl, raises: [].}
  setPaused {.importc: "setPaused".}:
    proc(player: SamplePlayerPtr, flag: cint) {.cdecl, raises: [].}

# type PlaydateSoundSampleplayer* = ptr PlaydateSoundSampleplayerRaw

type PlaydateSoundSequence {.
  importc: "const struct playdate_sound_sequence", header: "pd_api.h", bycopy
.} = object
  newSequence {.importc: "newSequence".}: proc(): SoundSequencePtr {.cdecl, raises: [].}
  freeSequence {.importc: "freeSequence".}:
    proc(player: SoundSequencePtr) {.cdecl, raises: [].}
  loadMIDIFile {.importc: "loadMIDIFile".}:
    proc(soundSeq: SoundSequencePtr, path: cstring): cint {.cdecl, raises: [].}
  getTime {.importc: "getTime".}:
    proc(soundSeq: SoundSequencePtr): cuint {.cdecl, raises: [].}
  setTime {.importc: "setTime".}:
    proc(soundSeq: SoundSequencePtr, time: cuint): void {.cdecl, raises: [].}
  setLoops {.importc: "setLoops".}: proc(
    soundSeq: SoundSequencePtr, loopstart: cint, loopend: cint, loops: cint
  ): void {.cdecl, raises: [].}
  setTempo {.importc: "setTempo".}:
    proc(soundSeq: SoundSequencePtr, stepsPerSecond: cfloat): void {.cdecl, raises: [].}
  ...
  allNotesOff {.importc: "allNotesOff".}:
    proc(soundSeq: SoundSequencePtr): void {.cdecl, raises: [].}
  isPlaying {.importc: "isPlaying".}:
    proc(soundSeq: SoundSequencePtr): cint {.cdecl, raises: [].}
  getLength {.importc: "getLength".}:
    proc(soundSeq: SoundSequencePtr): cuint {.cdecl, raises: [].}
  play {.importc: "play".}: proc(
    soundSeq: SoundSequencePtr,
    finishCallback: SequenceFinishedCallback = nil,
    userdata: pointer = nil,
  ): void {.cdecl, raises: [].}
  stop {.importc: "stop".}: proc(soundSeq: SoundSequencePtr): void {.cdecl, raises: [].}
  getCurrentStep {.importc: "getCurrentStep".}:
    proc(soundSeq: SoundSequencePtr, timeOffset: ptr cint): cint {.cdecl, raises: [].}
  setCurrentStep {.importc: "setCurrentStep".}: proc(
    soundSeq: SoundSequencePtr, step: cint, timeOffset: cint, playNotes: cint
  ): void {.cdecl, raises: [].}
  getTempo {.importc: "getTempo".}:
    proc(soundSeq: SoundSequencePtr): cfloat {.cdecl, raises: [].}
...
sdktype:
  type PlaydateSound* {.importc: "const struct playdate_sound", header: "pd_api.h".} = object
    ...
    getHeadphoneState {.importc: "getHeadphoneState".}: proc(
      headphone: ptr cint,
      headsetmic: ptr cint,
      changeCallback: proc(headphone: cint, mic: cint) {.cdecl, raises: [].},
    ) {.cdecl, raises: [].}
    setOutputsActive {.importc: "setOutputsActive".}:
      proc(headphone: cint, speaker: cint) {.cdecl, raises: [].} ##  1.5
