...
func toNimReturn(typeSymbol: string, call: NimNode): seq[NimNode] =
  ...
  of "cint", "cuint", "cfloat", "csize_t":
    return @[nnkReturnStmt.newTree(nnkCall.newTree(newIdentNode(newTypeSymbol), call))]
  of "cstring":
...
func unwrapType(typ: NimNode): NimNode =
  ...
  of nnkIdent, nnkSym:
    return typ
  of nnkPtrTy, nnkVarTy:
    return typ[0].unwrapType
  else:
    error("Unable to extract type of " & typ.lispRepr, typ)

func adjustRawProcIdentDef(
    identDef: NimNode, rawProcName: string, procName: string
): NimNode =
  let identDef = identDef.copy()

  var pragmas = identDef.findChild(it.kind == nnkProcTy).findChild(it.kind == nnkPragma)
  if pragmas == nil:
  ...
  var adjustedPragmas = nnkPragma.newTree(toAddPragmas)
  adjustedPragmas.add(
    newIdentNode("cdecl"),
  ...
  return nnkIdentDefs.newTree(
    ...
    nnkPragmaExpr.newTree(
      ...
      nnkPragma.newTree(
        nnkExprColonExpr.newTree(newIdentNode("importc"), newLit(procName))
      ),
    ),
    nnkProcTy.newTree(
      # Add params.
      identDef.findChild(it.kind == nnkProcTy).findChild(it.kind == nnkFormalParams),
...
proc generateSDKPtrProcDef*(
    rawProcDef: NimNode, rawProcName: string, typeName: string, procName: string
): NimNode =
  ...
  let newCall =
    nnkCall.newTree(nnkDotExpr.newTree(newIdentNode("this"), newIdentNode(rawProcName)))
    ...
    if param.kind == nnkEmpty:
      ...
      newProcParams.add(
        ...
        nnkIdentDefs.newTree(
          newIdentNode("this"), nnkPtrTy.newTree(newIdentNode(typeName)), newEmptyNode()
        ),
      )
      ...
      let identDefs = nnkIdentDefs.newTree(
        newIdentNode(param[0].strVal), newIdentNode(typeSymbol), newEmptyNode()
      )
  ...
  let procDef = nnkProcDef.newTree(
    nnkPostfix.newTree(newIdentNode("*"), newIdentNode(procName)),
    newEmptyNode(),
    newEmptyNode(),
    newProcParams,
    newProcPragmas,
...
proc processSDKType(ast: NimNode): NimNode =
  ...
  var recList =
    newAst[0][0].findChild(it.kind == nnkObjectTy).findChild(it.kind == nnkRecList)
      ...
      for idOrPragma in identAndPragmas:
        ...
        if idOrPragma.kind == nnkPragma and idOrPragma[0].kind == nnkIdent:
          ...
          if idOrPragma[0].strVal == "importsdk" and identDef[1].kind == nnkProcTy:
            ...
            var procName: string
            let identProcName = identDef.findChild(it.kind == nnkPragmaExpr).findChild(
                it.kind == nnkIdent
              )
            if identProcName != nil:
              ...
              procName =
                identDef.findChild(it.kind == nnkPragmaExpr).findChild(
                  it.kind == nnkPostfix
                )[1].strVal
            let rawProcName = procName # & "Raw"
            let adjustedRawProcIdentDef =
              adjustRawProcIdentDef(identDef, rawProcName, procName)
            ...
            let generatedProcDef =
              generateSDKPtrProcDef(identDef, rawProcName, typeName, procName)
