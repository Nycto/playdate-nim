...
# Doesn't work
type JSONValueUnion* {.
  importc: "json_value::no_name", header: "pd_api.h", bycopy, union
.} = object
...
type JSONDecoder* {.importc: "json_decoder", header: "pd_api.h".} = object
  decodeError* {.importc: "decodeError".}:
    proc(decoder: ptr JSONDecoder, error: cstring, linenum: cint) {.cdecl.}
    ##  the following functions are each optional
  willDecodeSublist* {.importc: "willDecodeSublist".}:
    proc(decoder: ptr JSONDecoder, name: cstring, `type`: JSONValueType) {.cdecl.}
  shouldDecodeTableValueForKey* {.importc: "shouldDecodeTableValueForKey".}:
    proc(decoder: ptr JSONDecoder, key: cstring): cint {.cdecl.}
  didDecodeTableValue* {.importc: "didDecodeTableValue".}:
    proc(decoder: ptr JSONDecoder, key: cstring, value: JSONValue) {.cdecl.}
  shouldDecodeArrayValueAtIndex* {.importc: "shouldDecodeArrayValueAtIndex".}:
    proc(decoder: ptr JSONDecoder, pos: cint): cint {.cdecl.}
  didDecodeArrayValue* {.importc: "didDecodeArrayValue".}:
    proc(decoder: ptr JSONDecoder, pos: cint, value: JSONValue) {.cdecl.}
    ##  if pos==0, this was a bare value at the root of the file
  didDecodeSublist* {.importc: "didDecodeSublist".}: proc(
    decoder: ptr JSONDecoder, name: cstring, `type`: JSONValueType
  ): pointer {.cdecl.}
  userdata* {.importc: "userdata".}: pointer
  returnString* {.importc: "returnString".}: cint
...
type JSONReader* {.importc: "json_reader", header: "pd_api.h".} = object
  read* {.importc: "read".}:
    proc(userdata: pointer, buf: ptr uint8, bufsize: cint): cint {.cdecl.}
...
type
  ...
  JSONEncoder* {.importc: "json_encoder", header: "pd_api.h".} = object
    ...
    addArrayMember* {.importc: "addArrayMember".}:
      proc(encoder: ptr JSONEncoder) {.cdecl.}
    ...
    addTableMember* {.importc: "addTableMember".}:
      proc(encoder: ptr JSONEncoder, name: cstring, len: cint) {.cdecl.}
    ...
    writeInt* {.importc: "writeInt".}:
      proc(encoder: ptr JSONEncoder, num: cint) {.cdecl.}
    writeDouble* {.importc: "writeDouble".}:
      proc(encoder: ptr JSONEncoder, num: cdouble) {.cdecl.}
    writeString* {.importc: "writeString".}:
      proc(encoder: ptr JSONEncoder, str: cstring, len: cint) {.cdecl.}

sdktype:
  type PlaydateJSON* {.importc: "const struct playdate_json", header: "pd_api.h".} = object
    initEncoder {.importc: "initEncoder".}: proc(
      encoder: ptr JSONEncoder, write: ptr WriteFunc, userdata: pointer, pretty: cint
    ) {.cdecl, raises: [].}
    decode {.importc: "decode".}: proc(
      functions: ptr JSONDecoder, reader: JSONReader, outval: ptr JSONValue
    ): cint {.cdecl, raises: [].}
    decodeString {.importc: "decodeString".}: proc(
      functions: ptr JSONDecoder, jsonString: cstring, outval: ptr JSONValue
    ): cint {.cdecl, raises: [].}
