...
type
  ...
  PlaydateLua* {.importc: "const struct playdate_lua", header: "pd_api.h", bycopy.} = object
    ...
    addFunction {.importc: "addFunction".}: proc(
      f: LuaNimFunction, name: cstring, outErr: ptr cstring
    ): cint {.cdecl, raises: [].}
    ...
    getArgType {.importc: "getArgType".}:
      proc(pos: cint, outClass: ptr cstring): LuaType {.cdecl, raises: [].}
    ...
    getArgFloat {.importc: "getArgFloat".}:
      proc(pos: cint): cfloat {.cdecl, raises: [].}
    getArgString {.importc: "getArgString".}:
      proc(pos: cint): cstring {.cdecl, raises: [].}
    getArgBytes {.importc: "getArgBytes".}:
      proc(pos: cint, outlen: ptr csize_t): cstring {.cdecl, raises: [].}
    getArgObject {.importc: "getArgObject".}:
      proc(pos: cint, `type`: cstring, outud: ptr ptr LuaUDObject): pointer {.cdecl.}
    ...
    pushBytes {.importc: "pushBytes".}:
      proc(str: cstring, len: csize_t) {.cdecl, raises: [].}
    pushBitmap {.importc: "pushBitmap".}: proc(bitmap: LCDBitmapPtr) {.cdecl.}
    pushSprite {.importc: "pushSprite".}: proc(sprite: LCDSpritePtr) {.cdecl.}
    pushObject {.importc: "pushObject".}:
      proc(obj: pointer, `type`: cstring, nValues: cint): ptr LuaUDObject {.cdecl.}
    retainObject {.importc: "retainObject".}:
      proc(obj: ptr LuaUDObject): ptr LuaUDObject {.cdecl.}
    releaseObject {.importc: "releaseObject".}: proc(obj: ptr LuaUDObject) {.cdecl.}
    setUserValue {.importc: "setUserValue".}:
      proc(obj: ptr LuaUDObject, slot: cuint) {.cdecl.}
    ##  sets item on top of stack and pops it
    getUserValue {.importc: "getUserValue".}:
      proc(obj: ptr LuaUDObject, slot: cuint): cint {.cdecl.}
    ##  pushes item at slot to top of stack, returns stack position
    ##  calling lua from C has some overhead. use sparingly!
    callFunction {.importc: "callFunction".}:
      proc(name: cstring, nargs: cint, outerr: ptr cstring): cint {.cdecl, raises: [].}
