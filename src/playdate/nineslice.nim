...
type
  NineSliceRow {.byref.} = object ## The bytes for a single row within a nine slice
    leftBytes: seq[uint8]
    middleBytes: seq[uint8]
    rightBytes: seq[uint8]
    leftRightBitLen: int

  NineSliceData = ref object ## Stores the rows for a single nine slice bitmap
    top: seq[NineSliceRow]
    middle: seq[NineSliceRow]
    bottom: seq[NineSliceRow]

  NineSlice* = ref object ## A precalculated nine slice
    image: NineSliceData
    mask: NineSliceData
...
func fillNineSliceRow(
    row: var NineSliceRow, source: BitmapData, y, leftRightBitLen: int
) =
  ...
  copyBits(
    source.data, row.leftBytes, sourceStartBit, 0, leftRightBitLen, 0, leftRightBitLen
  )
  ...
  copyBits(
    source.data,
    row.rightBytes,
    sourceStartBit + source.width - leftRightBitLen,
    0,
    leftRightBitLen,
    0,
    leftRightBitLen,
...
func createNineSliceData(source: BitmapData): NineSliceData =
  ...
  for i in 0 ..< sliceHeight:
    ...
    result.bottom[i].fillNineSliceRow(
      source, source.height - sliceHeight + i, leftRightBitLen
    )
  ...
  return NineSlice(
    ...
    mask:
      if source.getBitmapMask.resource == nil:
        ...
        createNineSliceData(source.getBitmapMask.getData),
  )
...
func drawRow(target: var BitmapData, source: NineSliceRow, targetY: int) =
  ...
  for i in 0 ..< middleBytesLen:
    target.data[i + targetOffsetByte + source.leftBytes.len] =
      source.middleBytes[i mod source.middleBytes.len]
  ...
  for i in 0 .. (imageWidthInBytes - rightColStartByte):
    ...
    target.data[targetOffsetByte + rightColStartByte + i] =
      overlapBytes(existingByte, rightByte, overlapBits)
