...
proc expectData[N: static int](
    map: var StaticSparseMap[N, int, string], expect: openarray[int]
) =
...
suite "StaticSparseMap":
  ...
  test "Bulk operations":
    ...
    for line in lines("tests/sparsemap_ops.txt"):
      ...
      checkpoint ${
        "action": parts[0], "removeKey": $removeKey, "setKey": $setKey, "value": $value
      }
      case parts[0]
      of "alloc":
